    ...
    var tableIns = table.render({
        ...
        , cols: [
            [
                ...
                , {field: 'commodity_name', title: '商品名称', width: 340}
                , {field: 'commodity_code', title: '商品编码', width: 150}
                , {field: 'specification_model', title: '规格型号', width: 190}
                , {field: 'measuring_unit', title: '计量单位名称', width: 180}
                , {field: 'shipping_fee', title: '配送费(元)', width: 180, sort: true}
                , {fixed: 'right', title: '操作', width: 200, align: 'center', toolbar: '#barOption'} //这里的toolbar值是模板元素的选择器
            ]
        ]
        , id: 'dataCheck'
        , url: '/GetMoney'
        // , where: {commodityName: commodity_name, commodityCode: commodity_code}
        , method: 'get'
        , page: true
    ...
    table.on('tool(delivery_fee)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
        ...
        if (layEvent === 'detail') { //查看
            ...
            layer.confirm('真的要审核吗', function (index) {
                ...
                layer.close(index);
                //向服务端发送删除指令
                alert(data.fk_synch_order_id)
                var params = {"fk_synch_order_id": data.fk_synch_order_id};
                var deleteIndex = layer.load(1, {shade: [0.1, '#fff']});
                $.ajax({
