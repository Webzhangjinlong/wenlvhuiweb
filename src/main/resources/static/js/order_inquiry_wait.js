...
layui.use(['laydate','table', 'form', 'layer', 'vip_table'], function () {
    ...
    table.render({
        elem: '#daipeisong'
        ,id:'testReload'
        ,url:'/order/mangerGetOrders?orderStatues=WAIT_GOODS_RECEIVE_CONFIRM'
        ,title: '待分配订单'
        ...
        ,cols: [[
            ...
            , {field: 'oi_order_start_time', title: '订单日期', width: 200}
        ]]
        ,page: true
...
function daochu(){
    ...
    var endDate = $('#endDate').val();
    var productName = $('#productName').val();
    var _url = "/order/export_order.json?orderStatues=WAIT_GOODS_RECEIVE_CONFIRM&demoReload="+demoReload+"&startDate="+startDate+"&endDate="+endDate+"&page="+page+"&limit="+limit+"&productName="+productName+"&city="+city+"&userName="+userName;
    location.href = _url;
}